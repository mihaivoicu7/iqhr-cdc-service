...
Direct environment table-list and source-identity changes are blocked after activation. Enabling SQL Server CDC alone discovers a table but does not select it for capture. Use **CDC → Tabele CDC** in Admin to request activation through the [durable table registry](docs/adding-tables.md). The service preserves its global checkpoint and establishes a separate, post-request activation boundary for the new table; earlier changes are not backfilled. Capture replacement or unavailable continuity blocks the affected source visibly. An initialized registry cannot silently bootstrap again after its rows are lost. The TECHNO TEST configuration includes `dbo.HR_Employee`, `dbo.HR_EmployeeContractInfo`, `dbo.HR_EmployeeAllocation` and `dbo.HR_EmployeeAddress`. The two synthetic acceptance tables and their 594 history events were removed through the [reviewed cutover](docs/test-cutover.md), preserving the existing checkpoint and source incarnation. Do not rerun probe provisioning against this cleaned deployment or mutate real employee rows as a test without explicit authorization. Historical acceptance results remain in [live deployment evidence](docs/deployment-evidence.md).

## Native SQL assessment

Application history inserts, deduplication, activation/fence lifecycle, table inventory and enable requests, status writes and bounded one-year retention use mapped JPA entities and entity transactions. Native SQL remains only for these concrete SQL Server/library contracts:

* `SqlServerControl`: `sp_getapplock`/`APPLOCK_MODE`, CDC metadata and LSN functions, documented `sp_cdc_help_jobs`, SQL Agent activity and storage/scan diagnostics. These server procedures/DMVs have no equivalent stable table-backed JPA lifecycle. Table names are bound as parameters; the heartbeat barrier and boundary checks use validated identifiers from the explicit allowlist. Capture-generation fingerprints include capture identity, column shape and ordered primary-key membership.
...
* `V4__durable_table_onboarding.sql`: guarded additive registry DDL and initialization marker, validated before JPA startup. No business table or CDC instance is recreated.
* `scripts/cleanup-test-probes.mjs` reads a private state backup through SQL Server tooling. Its standalone `scripts/maintenance/CleanupTestProbes.java` operator uses mapped JPA for synthetic history deletion and activation updates. Native SQL is retained for the newly included table's CDC boundary check, exclusive empty-fixture checks and permanent removal of the two explicitly authorized probe tables/capture instances. This is an explicit one-off TEST operation, never a startup migration.
* `scripts/onboard-test-tables.mjs` and `scripts/maintenance/OnboardTestTables.java`: explicit TEST onboarding/replay uses JPA for activation and the reviewed offset change, preserving history and source incarnation. Native SQL only reads the retained heartbeat and business CDC positions required to prove the replay boundary safe; identifiers are fixed and values are bound. The normal runtime still leaves all offset writes to Debezium.
